...
angular.module('GO.softDeleteButton',['GO.data']).
		directive('imSoftDeleteButton', ['MessageBox', 'Translate', function(MessageBox,Translate) {
				...
				return {
					...
					template: '<span ng-disabled="imDisabled" ng-if="!imModel.deleted" class="btn btn-{{imSize}} btn-danger im-soft-delete-btn" ng-click="delete($event);" tooltip="{{\'Delete\' | goT}}" tooltip-placement="left">\
						<i class="fa fa-trash-o"></i>\
					</span>\
					\
					<span ng-disabled="imDisabled" ng-if="imModel.deleted" class="btn btn-{{imSize}} btn-primary im-soft-delete-btn" ng-click="unDelete($event);" tooltip="{{\'Undo delete\' | goT}}" tooltip-placement="left">\
