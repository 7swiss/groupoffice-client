					...
					$scope.ok = function() {
						...
						for (var i = 0, c = shareModal.store.items.length; i < c; i++) {
							...
							if (model.isModified()) {
								...
								var record = {roleId: model.id};

								for (var n = 0, c2 = shareModal.permissionAttributes.length; n < c2; n++) {
									record[shareModal.permissionAttributes[n].name] = model.permissions[shareModal.permissionAttributes[n].name] === true;
								}
...
//						$scope.loading = true;
					...
					$scope.checkDependencies = function(role) {
						...
						disabledAttributes[role.id] = [];

						var permissionAttributes = role.permissions.attributes;
...
//console.log(attr);
								...
								for (var i = 0, c = permissionDependencies.length; i < c; i++) {
									if (disabledAttributes[role.id].indexOf(permissionDependencies[i]) === -1) {
										//set it checked and disabled
										disabledAttributes[role.id].push(permissionDependencies[i]);
									}
						...
						$scope.permissionDisabled = function(role, permissionAttribute) {
							return disabledAttributes[role.id] && disabledAttributes[role.id].indexOf(permissionAttribute) > -1;
						};
					};
